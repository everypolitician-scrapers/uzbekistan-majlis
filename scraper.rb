...
def scrape_person(url, data_i)
  ...
  data = { 
    ...
    region: info.xpath('.//li[contains(.,"Region:")]').text.split(':',2).last.to_s.tidy,
    constituency: info.xpath('.//li[contains(.,"Constituency:")]').text.split(':',2).last.to_s.tidy,
    party: info.xpath('.//li[contains(.,"Party:")]').text.split(':',2).last.to_s.tidy,
    faction: info.xpath('.//li[contains(.,"Faction member:")]').text.split(':',2).last.to_s.tidy,
    term: 5,
    source: url.to_s,
  }.merge(data_i)
